...
데이터 파이프라인, 자동화 봇 등 **상태 머신(State Machine) 기반의 멀티 에이전트 프로젝트**라면, **PRD(제품 요구사항 정의서) 마크다운 문서** 하나만 작성해서 이 공장에 넣으세요. 
...
# [선택] PRD 파싱 및 코드 생성에 사용할 LLM 엔진 (기본값: hybrid)
# hybrid : gpt-4o-mini(저비용) + deepseek-chat(고지능) 조합 → OPENAI_API_KEY + DEEPSEEK_API_KEY 필요
# claude : Claude (Anthropic) 단독 사용             → ANTHROPIC_API_KEY 필요
# openai : GPT-4o 단독 사용                         → OPENAI_API_KEY 필요
LLM_ENGINE=hybrid
DEEPSEEK_API_KEY="sk-..."
# ANTHROPIC_API_KEY는 claude 엔진 선택 시 활성화
```
...
> 💡 **강력 추천: 수동 작성 대신 전용 프롬프트를 사용하세요!**
> PRD의 엄격한 표 양식을 직접 타이핑하기보다, 제공되는 **[PRD 자동 생성 프롬프트(`docs/prd_template_prompt.md`)](docs/prd_template_prompt.md)** 문서를 활용하는 것을 권장합니다. 
> ChatGPT나 Claude에 해당 프롬프트를 복사해 넣고 아이디어만 적어주면, 공장 규격에 100% 완벽하게 맞는 마크다운 PRD를 대신 작성해 줍니다. (신규 생성 및 기존 PRD 수정 모두 지원)

이 공장이 코드를 정확히 생성하려면 수동이든 자동이든 PRD 파일(`docs/prd/my_project_prd.md`) 내에 **반드시 표 형태**로 다음 내용이 포함되어야 합니다.
...
> | 문서 | 공장 가동 여부 | 역할 |
> |------|:---:|------|
> | `master_workflow_prd.md` | ❌ 공장 입력 아님 | 전체 설계 지도 — 개발자와 AI가 참고하는 레퍼런스. 사람이 직접 관리. |
> | `docs/prd/0N_xxx_prd.md` (Micro-PRD) | ✅ 공장 입력 대상 | 기능 하나의 세부 명세 → 공장이 YAML 명세서 + 코드를 자동 생성 |
>
> `master_workflow_prd.md`에는 **공장을 돌리지 않습니다.** 하네스 공장은 **Micro-PRD 하나씩만** 입력으로 받습니다.
> 예외: 모든 Micro-PRD 구현 완료 후 전체를 하나로 조립하는 **5단계(통합 Auto-Wiring)** 시에만 한 번 더 가동합니다.


### 📝 단계별 작업 절차 (Bottom-Up 기획 방식)

1. **가벼운 전체 뼈대 작성 (Main-PRD 골격)** — ⚠️ 공장 미가동
   - 프로젝트 전체 목표와 예상 에이전트 흐름을 담은 가벼운 `master_workflow_prd.md`를 먼저 작성합니다.
   - **이 파일은 공장에 입력하지 않습니다.** 개발자와 AI가 전체 흐름을 파악하기 위한 '설계 지도' 역할만 합니다.
   - 구체적인 데이터 규격, 함수 시그니처는 비워두며, Micro-PRD 완료 시마다 역으로 채워나갑니다.
2. **Micro-PRD 작성 및 하네스 가동 (기능별 바텀업)** — ✅ 공장 가동 대상
   - 구현하려는 에이전트 하나의 세부 요구사항을 담은 Micro-PRD(예: `docs/prd/01_pre_locking_agent_prd.md`)를 작성합니다.
   - **중요:** 공장 가동 전 `.env`의 `TARGET_DOMAIN`을 해당 타겟 접두사로 변경합니다. (`_domain_spec.yaml` 제외. 예: `pre_locking_agent`)
   - 스크립트를 실행하면 공장이 Micro-PRD를 파싱 → YAML 명세서 생성 → 파이썬 코드를 자동 패치합니다.
